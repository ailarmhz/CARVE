...
def maybe_expand_multiclass_labels(df: pd.DataFrame) -> pd.DataFrame:
    """
    If CSV is [id_col, label] with integer class ids (e.g. 0/1/2),
    convert to one-hot columns [class_0, class_1, ...].
    """
    if df.shape[1] != 2:
        return df
    ...
    if vals.isna().any():
        return df
    uniq = sorted(set(int(v) for v in vals.tolist()))
    if len(uniq) <= 2:
        return df
    ...
    id_col = df.columns[0]
    out = pd.DataFrame({id_col: df[id_col].astype(str)})
    for c in uniq:
        out[f"class_{c}"] = (vals.astype(int) == c).astype(np.float32)
    return out
...
def find_volume_path_ctrate(data_root: str, volume_name: str) -> str:
    """Find NIfTI volume under data_root (or fallback Validation/Data). volume_name e.g. 'valid_1_a_1.nii.gz'."""
    base = volume_name.replace(".nii.gz", "").replace(".nii", "")
    # Try primary root and common fallbacks (validation often in Validation/Data)
    roots_to_try = [
        data_root,
        os.path.join(data_root, "Validation", "Data"),
        os.path.join(os.path.dirname(data_root.rstrip(os.sep)), "Validation", "Data"),
    ]
    for root in roots_to_try:
        if not root or not os.path.isdir(root):
            continue
        for ext in (".nii.gz", ".nii"):
            direct = os.path.join(root, base + ext)
            if os.path.exists(direct):
                return direct
        for ext in (".nii.gz", ".nii"):
            pattern = os.path.join(root, "**", base + ext)
            matches = glob.glob(pattern, recursive=True)
            if matches:
                return matches[0]
    raise FileNotFoundError(
        f"Could not find NIfTI for {volume_name} under {data_root} (tried Validation/Data fallbacks). "
        "Set CTRATE_DATA_ROOT to the directory that contains valid_*.nii.gz (e.g. /datasets/ctrate/Validation/Data)."
    )


def _file_stem(path: str) -> str:
    """Filename without extension; treat .nii.gz as one extension."""
...
def find_volume_path_luna(data_root: str, image_id: str) -> str:
    """Find volume under data_root/subset0..subset9 (LUNA all_subsets). LUNA16 = MetaImage (.mhd/.raw). Match by exact base name, then by listing files."""
    base = image_id.replace(".nii.gz", "").replace(".nii", "").replace(".mhd", "").replace(".raw", "").strip()
    # CC-CCII style: image_id can be a directory containing 2D slices (e.g. png/jpg).
    direct_dir = os.path.join(data_root, image_id)
    if os.path.isdir(direct_dir):
        pngs = (
            glob.glob(os.path.join(direct_dir, "*.png"))
            + glob.glob(os.path.join(direct_dir, "*.PNG"))
            + glob.glob(os.path.join(direct_dir, "*.jpg"))
            + glob.glob(os.path.join(direct_dir, "*.JPG"))
            + glob.glob(os.path.join(direct_dir, "*.jpeg"))
            + glob.glob(os.path.join(direct_dir, "*.JPEG"))
        )
        if pngs:
            return direct_dir
    exts_to_try = [".mhd", ".nii.gz", ".nii"]
    # 1) Exact path
    for i in range(10):
        sub = os.path.join(data_root, f"subset{i}")
        ...
        for ext in exts_to_try:
            ...
            if os.path.exists(p):
                return p
    # 2) List files in each subset (and one level down) and match by stem
    for i in range(10):
        sub = os.path.join(data_root, f"subset{i}")
        if not os.path.isdir(sub):
            continue
        for ext in exts_to_try:
            for pattern in [os.path.join(sub, "*" + ext), os.path.join(sub, "*", "*" + ext)]:
                for p in glob.glob(pattern):
                    if not os.path.isfile(p):
                        continue
                    if _file_stem(p) == base:
                        return p
    # 3) Generic recursive fallback (for non-LUNA layouts such as CC-CCII demo folders)
    # 3a) Recursive directory fallback for stacked 2D slices.
    for d in glob.glob(os.path.join(data_root, "**", base), recursive=True):
        if os.path.isdir(d):
            pngs = (
    ...
    # 3b) Recursive file fallback for medical volume files.
    for ext in exts_to_try:
    ...
    raise FileNotFoundError(
        f"Could not find volume for {image_id} (tried .mhd, .nii.gz, .nii; exact and stem match) under {data_root}. "
        "For LUNA, set LUNA_DATA_ROOT to all_subsets (subset0..9). "
        "For CC-CCII, set CCII_DATA_ROOT to the folder that contains (or recursively contains) the volume files."
    )


def load_nifti_volume(path: str) -> np.ndarray:
    if nib is None:
        raise ImportError("nibabel is required for NIfTI loading. pip install nibabel")
    img = nib.load(path)
    arr = np.asarray(img.get_fdata(), dtype=np.float32)
    return arr


def get_nifti_spacing(path: str) -> Tuple[Optional[float], Optional[float], Optional[float]]:
    """Return (z_spacing, xy_spacing, xy_spacing) from NIfTI header."""
    if nib is None:
    ...
    if len(zooms) < 3:
        return None, None, None
    z_spacing = float(zooms[0])
    xy_spacing = float((zooms[1] + zooms[2]) / 2.0)
    return z_spacing, xy_spacing, xy_spacing


def load_mhd_volume(path: str) -> np.ndarray:
    """Load MetaImage (.mhd), NIfTI, or directory of 2D slices (png/jpg/jpeg)."""
    if os.path.isdir(path):
        ...
        if not files:
            raise FileNotFoundError(f"No png/jpg/jpeg slices found in directory: {path}")
        slices = []
        for fp in files:
            img = Image.open(fp).convert("L")
            slices.append(np.asarray(img, dtype=np.float32))
        return np.stack(slices, axis=0)

    path_lower = path.lower()
    if path_lower.endswith(".mhd") or path_lower.endswith(".raw"):
        if sitk is None:
            raise ImportError("SimpleITK is required for LUNA16 MHD/RAW. pip install SimpleITK")
    ...
    # Try SimpleITK for unknown extension (e.g. .mhd)
    if sitk is not None:
        img = sitk.ReadImage(path)
        arr = np.asarray(sitk.GetArrayFromImage(img), dtype=np.float32)
        return arr
...
def get_mhd_spacing(path: str) -> Tuple[Optional[float], Optional[float], Optional[float]]:
    """Return (z_spacing, xy_spacing, xy_spacing) from MHD or NIfTI. SimpleITK GetSpacing is (x,y,z)."""
    if os.path.isdir(path):
        # No physical spacing metadata for folder-of-slices datasets (e.g. CC-CCII png stacks).
        return None, None, None
    path_lower = path.lower()
    if path_lower.endswith(".mhd") or path_lower.endswith(".raw"):
        if sitk is None:
        ...
        if len(sp) < 3:
            return None, None, None
        z_spacing = float(sp[2])
        xy_spacing = float((sp[0] + sp[1]) / 2.0)
        return z_spacing, xy_spacing, xy_spacing
...
def _extract_spacing_and_rescale_ctrate(row: pd.DataFrame):
    """CT-RATE metadata: RescaleSlope, RescaleIntercept, XYSpacing '[x,y]', ZSpacing."""
    if row is None or len(row) == 0:
        return None, None, None, None
    slope = row["RescaleSlope"].iloc[0] if "RescaleSlope" in row.columns else None
    intercept = row["RescaleIntercept"].iloc[0] if "RescaleIntercept" in row.columns else None
    xy_raw = row["XYSpacing"].iloc[0] if "XYSpacing" in row.columns else None
    z = row["ZSpacing"].iloc[0] if "ZSpacing" in row.columns else None
    xy = None
    if xy_raw is not None and isinstance(xy_raw, str):
        m = _float_pat.findall(str(xy_raw))
        xy = float(m[0]) if m else None
    elif xy_raw is not None:
        xy = _parse_float_any(xy_raw)
    return (
        _parse_float_any(slope),
        _parse_float_any(intercept),
        xy,
...
class CTRateNIfTIDataset(Dataset):
    """CT-RATE dataset: NIfTI volumes under data_root, labels and metadata from CSV."""
    ...
    def __getitem__(self, idx: int):
        row = self.df.iloc[idx]
        sid = str(row[self.id_col])  # e.g. valid_1_a_1.nii.gz

        row_metadata = self.df_metadata[self.df_metadata["VolumeName"] == sid]
        slope, intercept, xy_spacing, z_spacing = _extract_spacing_and_rescale_ctrate(row_metadata)

        vol_path = find_volume_path_ctrate(self.data_root, sid)
        vol = load_nifti_volume(vol_path)
        vol = _ensure_dhw(vol)
...
class LUNANIfTIDataset(Dataset):
    """LUNA dataset: volumes under data_root/subset0..subset9 (LUNA16 = MetaImage .mhd/.raw), labels from CSV (image_id, label 0/1). Spacing from image header."""
    ...
    def __getitem__(self, idx: int):
        ...
        vol_path = find_volume_path_luna(self.data_root, sid)
        vol = load_mhd_volume(vol_path)  # LUNA16 = .mhd/.raw; also supports .nii
        vol = _ensure_dhw(vol)

        z_spacing, xy_spacing, _ = get_mhd_spacing(vol_path)
        slope, intercept = 1.0, 0.0  # LUNA often in HU; MHD may have rescale in header
...
def multilabel_metrics_from_probs(probs: torch.Tensor, y_true: torch.Tensor,
    ...
    # Binary (single-label): use best F1 threshold for main metrics (at 0.5 model often predicts no positives)
    if L == 1:
        best_f1, best_thr, best_prec, best_rec = 0.0, 0.5, 0.0, 0.0
        for t in np.linspace(0.05, 0.95, 19):
            p = (probs_np[:, 0] >= t).astype(np.int64)
            yj = y_np[:, 0]
            tp = int(((p == 1) & (yj == 1)).sum())
            fp = int(((p == 1) & (yj == 0)).sum())
            fn = int(((p == 0) & (yj == 1)).sum())
            ...
            f1 = (2 * prec * rec) / (prec + rec + 1e-12)
            if f1 >= best_f1:
                best_f1, best_thr, best_prec, best_rec = float(f1), float(t), float(prec), float(rec)
        summary["binary_best"] = {
            "threshold": best_thr,
            "f1": best_f1,
            "precision": best_prec,
        ...
        # Overwrite micro/macro F1, precision, recall with best-threshold values so main summary is meaningful
        summary["threshold"] = best_thr
        summary["threshold_note"] = "binary: threshold chosen to maximize F1 (was 0.5)"
        summary["micro"]["f1"] = best_f1
        summary["micro"]["precision"] = best_prec
        summary["micro"]["recall"] = best_rec
        summary["macro"]["f1"] = best_f1
        summary["macro"]["precision"] = best_prec
        summary["macro"]["recall"] = best_rec
        # Accuracy at best threshold
        p_best = (probs_np[:, 0] >= best_thr).astype(np.int64)
        acc_best = float((p_best == y_np[:, 0]).mean())
        summary["micro"]["accuracy"] = acc_best
        summary["macro"]["accuracy"] = acc_best
        summary["subset_acc"] = acc_best
        # Per-label too
        for name in per_label:
            per_label[name]["f1"] = best_f1
            per_label[name]["precision"] = best_prec
            per_label[name]["recall"] = best_rec
            per_label[name]["accuracy"] = acc_best
...
def estimate_k_hat_from_probs(probs: torch.Tensor, min_k: int = 1, max_k: int = 3):
    k_hat = torch.round(probs.sum(dim=1)).long()
    ...
    def forward_and_adapt(self, x: torch.Tensor, accumulate_only: bool = False, loss_scale: float = 1.0):
        ...
        with amp_ctx:
            ...
            if self.use_mltta_bem:
                ...
                # ----- No-grad: score views one-at-a-time (no list of view tensors kept) -----
                with torch.no_grad():
                    view_probs_ng = []
                    for i in range(V):
                        if i == 0:
                ...
                total_loss = 0.0
                for k in range(K):
                    view_idx = keep_ids[k, 0].item()  # use batch 0's selection for memory efficiency
                    if view_idx == 0:
                        xv = x
                    else:
                        xv = weak_3d_augment(
                    ...
                    loss_k, bem_stats_k = bem_topk_loss(
                ...
                struct_loss = None  # BEM path already stepped above; skip generic backward/step
...
class RunCfg:
    ...
    split: str
    results_dir: str
    dataset_type: str  # "radchest" | "ctrate"
    batch_size: int
    num_workers: int
    ...
    metadata_csv: str
    label_display_names: Optional[List[str]] = None  # for templates (e.g. LUNA: ["nodule"] when CSV has "label")
...
def run_zero_shot(cfg: RunCfg):
    ...
    if cfg.dataset_type == "ctrate":
        df_va = pd.read_csv(cfg.val_csv)
        df_tr = df_va  # use validation for priors when no train
        df_te = df_va
        df = df_va
        if cfg.split != "val":
            logger.warning("CT-RATE mode: using validation split only (--split val)")
    elif cfg.dataset_type == "luna":
        df_va = pd.read_csv(cfg.val_csv)
        df_tr = df_va
        df_te = df_va
        df = df_va
        if cfg.split != "val":
            logger.warning("LUNA mode: using single split (--split val)")
    else:
    ...
    df = maybe_expand_multiclass_labels(df)
    df_tr = maybe_expand_multiclass_labels(df_tr)

    id_col = df.columns[0]
    label_cols = list(df.columns[1:])
    prompt_label_names = cfg.label_display_names if cfg.label_display_names else label_cols
    ...
    logger.info(f"Dataset: {cfg.dataset_type} | Split: {cfg.split} | #samples={len(df)} | #labels={len(label_cols)}")

    if cfg.dataset_type == "ctrate":
        ds = CTRateNIfTIDataset(
            data_root=cfg.radchest_root,
            df=df,
            id_col=id_col,
    ...
    for step, (sids, x, y) in enumerate(dl):
        ...
        if cfg.use_tent:
            ...
            with torch.no_grad():
                B, L = x.shape[0], len(label_cols)
                probs_bl = torch.empty((B, L), device=device, dtype=torch.float32)
                for j in range(L):
                    tok_pos = repeat_batchencoding(pos_tok_single[j], B).to(device)
                    tok_neg = repeat_batchencoding(neg_tok_single[j], B).to(device)

                    sim_pos = model(tok_pos, x, device=device)
                    sim_neg = model(tok_neg, x, device=device)

                    pair_logits = torch.stack([sim_neg, sim_pos], dim=-1)
                    probs_bl[:, j] = F.softmax(pair_logits, dim=-1)[:, 1]
    ...
    if cfg.dataset_type == "ctrate" and y_true.sum().item() == 0:
        logger.warning(
            "CT-RATE: all labels (y_true) are zero. F1/precision/recall will be 0. "
            "Replace the labels CSV (e.g. valid_predicted_labels.csv) with real 0/1 labels for valid_* VolumeNames (same format as train_predicted_labels.csv) for meaningful metrics."
        )

    summary, per_label = multilabel_metrics_from_probs(probs, y_true, label_cols, thr=cfg.thr)
    if "binary_best" in summary:
        b = summary["binary_best"]
        logger.info(
            "Binary task: at thr=0.5 F1=0 is common if model scores are low; best F1=%.4f at thr=%.2f (precision=%.4f, recall=%.4f)",
            b["f1"], b["threshold"], b["precision"], b["recall"],
        )
    if cfg.use_tent:
        summary["tent"] = {
            "enabled": True,
...
def parse_args():
    ...
    p.add_argument("--dataset_type", type=str, default="radchest", choices=["radchest", "ctrate", "luna"],
                   help="radchest: NPZ. ctrate: NIfTI validation. luna: NIfTI under radchest_root/subset0..9, single CSV with image_id,label")
    p.add_argument("--luna_class_name", type=str, default="nodule", help="For dataset_type=luna: class name for prompts (CSV has 'label' 0/1)")
...
def main():
    ...
    # For dataset_type=luna, keep single display name only for true binary CSVs with [id, label] in {0,1}.
    # For multi-class CSVs (e.g. CC-CCII label in {0,1,2}), use expanded class columns directly.
    luna_label_display_names = None
    if args.dataset_type == "luna":
        try:
            _df_all = pd.read_csv(args.val_csv)
            if _df_all.shape[1] == 2:
                _vals = pd.to_numeric(_df_all.iloc[:, 1], errors="coerce")
                uniq = sorted(set(int(v) for v in _vals.dropna().tolist()))
                if _vals.notna().all() and len(uniq) <= 2 and set(uniq).issubset({0, 1}):
                    luna_label_display_names = [args.luna_class_name]
        except Exception:
            luna_label_display_names = [args.luna_class_name]
    ...
    cfg = RunCfg(
        ...
        label_display_names=luna_label_display_names,
