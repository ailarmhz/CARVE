...
def main():
    ...
    # For dataset_type=luna, keep single display name only for binary CSVs with [id, label].
    # For multi-class one-hot CSVs, use CSV column names directly.
    luna_label_display_names = None
    if args.dataset_type == "luna":
        try:
            _df_head = pd.read_csv(args.val_csv, nrows=8)
            if _df_head.shape[1] == 2:
                _vals = pd.to_numeric(_df_head.iloc[:, 1], errors="coerce")
                if _vals.notna().all() and sorted(set(int(v) for v in _vals.tolist())) in ([0, 1], [0], [1]):
